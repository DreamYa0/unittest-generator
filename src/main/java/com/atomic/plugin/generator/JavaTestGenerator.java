...
import com.atomic.plugin.java.Field;
import com.atomic.plugin.java.JavaAstModel;
import com.atomic.plugin.java.Method;
import com.atomic.plugin.java.Pair;
import com.atomic.plugin.parser.JavaParser;
...
import com.atomic.plugin.util.VelocityUtils;
...
public class JavaTestGenerator {
    ...
    private static String spliceFullClassPath(String className) {
        ...
        if (currentPath.contains("/") && currentPath.contains("/src/test")) {
            String newCurrentPath = currentPath.replace("/src/test", "/src/main/java/").replace("-test", "-api");
            String newClassName = className.replace(".", "/");
            return newCurrentPath + newClassName + ".java";
        } else if (currentPath.contains("\\") && currentPath.contains("\\src\\test")) {
            String newCurrentPath = currentPath.replace("\\src\\test", "\\src\\main\\java\\").replace("-test", "-api");
            String newClassName = className.replace(".", "\\");
            return newCurrentPath + newClassName + ".java";
        }
